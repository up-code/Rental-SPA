namespace Rental_SPA.Sitio {
    ...
    interface historiaParameters { Tabs: HistoriaRow[] }
    const HistoriaCard: React.SFC<historiaParameters> = ({ }) => {

        const [Tabs, setTabs] = React.useState<HistoriaRow[]>([]);

        React.useEffect(() => {
            Rental_SPA.Sitio.HistoriaService.List({}, resp => {
        ...
        return (<div className="tabs-custom tabs-horizontal tabs-line tabs-line-big tabs-line-style-2 text-center text-md-left" id="tabs-7">
            <ul className="nav nav-tabs mt-3">
                {Tabs.map((item, i) => {
                    return <> <li key={i} className="nav-item" role="presentation"><a className={"nav-link " + (i == 0 ? "active" : "")} href={"#tabs-7-" + i} data-toggle="tab">{item.Titulo}</a></li></>
                })}
            </ul>
            <div className="tab-content">
                {Tabs.map((item, i) => {
                    return <div key={i} className={"tab-pane fade " + (i == 0 ? "show active" : "")} id={"tabs-7-" + i}>
